...
def test_find_top_issues():
    DEFAULT_TOP = 10  # MUST CHANGE if default top=10 changes.
    ...
    top_outlier_indices_3 = rank.find_top_issues(quality_scores=ood_scores, top=3)
    assert len(top_outlier_indices_k) == 3

    # Get k = DEFUALT_TOP ood scores
    top_outlier_indices = rank.find_top_issues(ood_scores)
    assert len(top_outlier_indices) == DEFAULT_TOP

    # Assert top 3 scores are consistent with different length scores vectors
    assert (top_outlier_indices_3 == top_outlier_indices[:3]).all()  # scores consistent

    top_outlier_indices_0 = rank.find_top_issues(ood_scores, top=0)
    assert len(top_outlier_indices_0) == 0

    top_outlier_indices_1 = rank.find_top_issues(quality_scores=ood_scores, top=1)
